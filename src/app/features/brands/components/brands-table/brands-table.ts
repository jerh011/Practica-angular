...
export class BrandsTable {
  ...
  private readonly snapshot = computed(() => this.data().map((record) => structuredClone(record)));
  ...
  toggleVisibleInMenu(data: EntityData, newValue: boolean): void {
    ...
    if (!record) {
      this.service.brandStatusChange$.error(
        new Error(`No se encontró el registro para la marca con _recordKey: ${data._recordKey}`),
      );
    }
    if (record) {
